...
const AudioReportPage: React.FC = () => {
    ...
    const handleDownloadReport = () => {
        if (data.report_pdf_base64) {
            // Use the actual PDF generated by the backend
            const byteCharacters = atob(data.report_pdf_base64);
            ...
            a.href = url;
            a.download = `voice_analysis_report_${data.diagnosis.replace("'", "")}_${new Date().toISOString().split('T')[0]}.pdf`;
            document.body.appendChild(a);
            a.click();
            document.body.removeChild(a);
            URL.revokeObjectURL(url);
        } else {
            // Fallback to text report if PDF not available
            const reportContent = `
...
METHOD: Voice Feature Analysis (Ensemble Ensemble)
            ...
            <div className="max-w-7xl mx-auto">
                ...
                {data.model_metrics && (
                    <div className="bg-card border rounded-lg p-6 mb-8">
                        <h2 className="text-xl font-bold mb-4">Model Ensemble Analysis (4-Model Breakdown)</h2>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {data.model_metrics.map((model: any, idx) => (
                                <div key={idx} className="p-4 bg-gray-50 rounded-xl border border-gray-100">
                                    <div className="flex justify-between items-start mb-2">
                                        <div className="font-bold text-primary">{model.name}</div>
                                        <div className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${model.diagnosis === "Parkinson's" ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'}`}>
                                            {model.diagnosis || 'Success'}
                                        </div>
                                    </div>
                                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-3">
                                        <div>
                                            <div className="text-[10px] uppercase opacity-50 font-bold">Confidence</div>
                                            <div className="text-lg font-mono font-bold text-gray-800">{(model.probability * 100).toFixed(1)}%</div>
                                        </div>
                                        <div>
                                        ...
                                        <div>
                                            <div className="text-[10px] uppercase opacity-50 font-bold">Status</div>
                                            <div className="text-lg font-bold text-primary text-[11px] mt-1">{model.status}</div>
                                        </div>
                                    </div>
                                    <div className="mt-3 w-full bg-gray-200 rounded-full h-1">
                                        <div
                                            className={`h-1 rounded-full ${model.diagnosis === "Parkinson's" ? 'bg-red-500' : 'bg-green-500'}`}
